  ...
  <div>
    ...
    <template v-else-if="labour">
      <div class="mb-4 flex items-start justify-between gap-3">
        <div>
          <h1 class="text-xl font-bold text-white lg:text-gray-900">{{ labour.name }}</h1>
          <p class="ui-muted mt-0.5">
            Wage {{ labour.daily_wage }} · Earned {{ labour.earned_total }} · Paid
            {{ labour.paid_total }}
          </p>
          <p class="mt-0.5 text-sm font-semibold text-green-300 lg:text-green-700">Pending {{ labour.pending_wage }}</p>
        </div>
        <div class="flex shrink-0 flex-col gap-2 sm:flex-row">
          <NuxtLink
            :to="`/sites/${route.params.siteId}/crew/pay?labour_id=${labour.id}`"
