  ...
  <div>
    ...
    <template v-else-if="material">
      <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 class="text-xl font-bold text-gray-900">{{ material.name }}</h1>
      ...
      <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        <UiCard>
          ...
          <p class="ui-stat-value mt-1 text-gray-900">
            {{ fmtQty(material.total_quantity_used) }}
            <span class="text-sm font-normal text-gray-500">{{ material.unit_of_measure }}</span>
          </p>
        </UiCard>
        <UiCard>
          <p class="ui-label !mb-0">Total cost</p>
          <p class="mt-1 text-lg font-bold tabular-nums text-xl text-violet-700">
            {{ fmtMoney(material.total_amount_spent) }}
          </p>
        </UiCard>
        <UiCard>
          <p class="ui-label !mb-0">Paid</p>
          <p class="mt-1 text-lg font-bold tabular-nums text-gray-900">
            {{ fmtMoney(material.total_amount_paid) }}
          </p>
        </UiCard>
        <UiCard>
          <p class="ui-label !mb-0">Pending</p>
          <p class="mt-1 text-lg font-bold tabular-nums text-amber-600">
            {{ fmtMoney(material.pending_amount) }}
          </p>
        </UiCard>
        <UiCard>
          <p class="ui-label !mb-0">Entries</p>
          <p class="ui-stat-value mt-1 text-gray-900">{{ material.usage_count }}</p>
        </UiCard>
        <UiCard>
      ...
      <div class="lg:grid lg:grid-cols-12 lg:gap-8">
        <div class="lg:col-span-4">
          <UiCard class="space-y-4">
            <p class="text-sm font-semibold text-gray-800 text-gray-700">Edit material</p>
            <div>
            ...
            <p v-if="saveMsg" class="text-sm text-violet-700">{{ saveMsg }}</p>
        ...
        <div class="mt-6 lg:col-span-8 lg:mt-0">
          <div class="mb-3 flex items-center justify-between">
            <p class="text-sm font-semibold text-gray-600 text-gray-700">Usage history</p>
          ...
          <template v-else>
            <div v-if="usageEntries.length" class="space-y-2">
                ...
                <div class="flex items-center justify-between gap-3">
                  <div class="flex-1">
                    <div class="flex flex-wrap items-center gap-3">
                      <p class="text-sm font-semibold text-gray-900">{{ entry.usage_date }}</p>
                      ...
                      <p class="mt-1 text-xs text-gray-500">
                        ...
                        <span
                          class="font-semibold"
                          :class="parseAmount(entry.pending_amount) > 0 ? 'text-amber-600' : 'text-violet-700'"
                        >
              ...
              <div class="rounded-lg border border-violet-500/20 bg-violet-500/[0.06] px-4 py-3 border-violet-200 bg-violet-50">
                <p class="text-xs font-semibold uppercase tracking-widest text-violet-300 text-violet-600">Running total</p>
                <p class="mt-1 text-xl font-black tabular-nums text-violet-700">{{ fmtMoney(runningTotal) }}</p>
                <p class="ui-muted mt-0.5">{{ fmtQty(totalQty) }} {{ material.unit_of_measure }} total</p>
              </div>
            </div>

            <UiCard v-else class="text-center">
              <p class="text-gray-500">No usage entries yet</p>
            ...
            <div v-if="payments.length" class="mt-8">
              <p class="mb-3 text-sm font-semibold text-gray-600 text-gray-700">Payment history</p>
              <div class="space-y-2">
                <div
                  v-for="p in payments"
                  :key="p.id"
                ...
                >
                  <div class="flex items-center justify-between gap-3">
                    <div>
                      <p class="text-sm font-semibold text-gray-900">{{ p.payment_date }}</p>
                      <p class="text-xs text-gray-500">
                        {{ p.payment_type }} · balance after {{ fmtMoney(p.remaining_amount) }}
                      </p>
                      <p v-if="p.notes" class="mt-0.5 text-xs text-gray-400">{{ p.notes }}</p>
                    </div>
                    <p class="text-sm font-bold tabular-nums text-violet-700">
